...
class DocumentConverter:
    ...
    def __init__(self):
        ...
        self.output_formats = {
            'pdf', 'docx', 'html', 'markdown', 'rtf', 'odt', 'txt'
        }
    ...
    def convert_single_document(self, input_path: str, output_path: str, 
                              output_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if input_format == 'doc' and output_format != 'txt':
                return self._convert_doc_via_docx(input_path, output_path, output_format, options)
            
            # Route to appropriate conversion method
            if output_format == 'pdf':
                return self._convert_to_pdf(input_path, output_path, input_format, options)
            elif output_format == 'docx':
                return self._convert_to_docx(input_path, output_path, input_format, options)
            elif output_format == 'html':
                return self._convert_to_html(input_path, output_path, input_format, options)
            elif output_format == 'markdown':
                return self._convert_to_markdown(input_path, output_path, input_format, options)
            elif output_format == 'rtf':
                return self._convert_to_rtf(input_path, output_path, input_format, options)
            elif output_format == 'odt':
                return self._convert_to_odt(input_path, output_path, input_format, options)
            elif output_format == 'txt':
                return self._convert_to_txt(input_path, output_path, input_format, options)
            else:
    ...
    def merge_and_convert_documents(self, input_paths: List[str], output_path: str,
                                  output_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if output_format == 'txt' and merged_path.endswith('.txt'):
                ...
                return self.convert_single_document(merged_path, output_path, output_format, options)
                
        except Exception as e:
            logger.error(f"Error merging documents: {e}")
            return False, f"Merge error: {str(e)}"
    
    def _merge_as_text(self, input_paths: List[str], output_path: str, 
        ...
        try:
            ...
            for input_path in input_paths:
                ...
                success, error = self._convert_to_txt(input_path, temp_txt.name, 
                                                    self.detect_document_format(input_path), options)
                if success:
            ...
            with open(output_path, 'w', encoding=options.get('text_encoding', 'utf-8')) as f:
                f.write('\n'.join(merged_content))
    ...
    def _merge_as_html(self, input_paths: List[str], output_path: str,
                      options: Dict[str, Any]) -> Tuple[bool, str]:
        """Merge documents as HTML"""
        try:
            html_parts = []
            html_parts.append('<!DOCTYPE html>')
            html_parts.append('<html><head><meta charset="utf-8"><title>Merged Document</title>')
            html_parts.append('<style>body { font-family: Arial, sans-serif; margin: 40px; }</style>')
            ...
            for i, input_path in enumerate(input_paths):
                ...
                success, error = self._convert_to_html(input_path, temp_html.name,
                                                     self.detect_document_format(input_path), options)
                if success:
                    # Try different encodings for reading HTML files
                    content = None
                    for encoding in ['utf-8', 'utf-8-sig', 'latin-1', 'windows-1252', 'cp1252']:
                        try:
                            with open(temp_html.name, 'r', encoding=encoding) as f:
                                content = f.read()
                            break
                    ...
                    html_parts.append(f'<h1>Document {i+1}: {Path(input_path).name}</h1>')
    ...
    def _merge_via_html(self, input_paths: List[str], output_path: str,
                       options: Dict[str, Any]) -> Tuple[bool, str]:
        """Merge documents by converting all to HTML first"""
        return self._merge_as_html(input_paths, output_path, options)
    
    def _convert_to_pdf(self, input_path: str, output_path: str, input_format: str,
                       options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if input_format == 'docx' and self.dependencies['win32com']:
                try:
                    success, error = self._convert_docx_to_pdf_com(input_path, actual_output, options)
                    if success:
            ...
            if not conversion_success and self.dependencies['pandoc'] and input_format in ['docx', 'html', 'markdown', 'rtf', 'odt']:
                try:
                    success, error = self._convert_to_pdf_pandoc(input_path, actual_output, input_format, options)
                    if success:
                        conversion_success = True
                    else:
                        conversion_error = error
            ...
            if not conversion_success and self.dependencies['weasyprint']:
                try:
                    success, error = self._convert_to_pdf_weasyprint(input_path, actual_output, input_format, options)
                    if success:
                        conversion_success = True
                    else:
                        conversion_error = error
            ...
            if not conversion_success and self.dependencies['reportlab']:
                try:
                    success, error = self._convert_to_pdf_reportlab(input_path, actual_output, input_format, options)
                    if success:
                        conversion_success = True
                    else:
                        conversion_error = error
                except Exception as reportlab_error:
                    logger.warning(f"ReportLab PDF conversion failed: {reportlab_error}")
            ...
            if pdf_password and temp_pdf_path:
                success, error = self._apply_pdf_password_protection(temp_pdf_path, output_path, pdf_password)
    ...
    def _apply_pdf_password_protection(self, input_pdf_path: str, output_pdf_path: str, password: str) -> Tuple[bool, str]:
                ...
                try:
                    ...
                    if extracted_text.strip():
                        ...
                        try:
                            ...
                            logger.warning(f"PyPDF2 password protection in ReportLab fallback failed: {pypdf_error}")
                        os.unlink(temp_txt.name)
                        return True, ""
                    
                except Exception as reportlab_error:
                    logger.warning(f"ReportLab password protection failed: {reportlab_error}")
            
            # If all methods fail, just copy the file without password protection
            logger.warning("Password protection not available, copying PDF without protection")
            shutil.copy2(input_pdf_path, output_pdf_path)
            return True, "Password protection not available, PDF saved without protection"
            
        except Exception as e:
            logger.error(f"PDF password protection error: {e}")
            return False, str(e)
    
    def _convert_to_pdf_weasyprint(self, input_path: str, output_path: str, 
                                  input_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if input_format != 'html':
                temp_html = tempfile.NamedTemporaryFile(mode='w+', suffix='.html', delete=False)
                temp_html.close()
                success, error = self._convert_to_html(input_path, temp_html.name, input_format, options)
                if not success:
                    os.unlink(temp_html.name)
    ...
    def _convert_to_pdf_reportlab(self, input_path: str, output_path: str,
                                 input_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            success, error = self._convert_to_txt(input_path, temp_txt.name, input_format, options)
            if not success:
                os.unlink(temp_txt.name)
            ...
            if pdf_password:
                ...
                success, error = self._apply_pdf_password_protection(output_path, output_path, pdf_password)
                if not success:
    ...
    def _convert_to_pdf_pandoc(self, input_path: str, output_path: str,
                              input_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            try:
                ...
                if "geometry" in str(geometry_error).lower() or "keyval" in str(geometry_error).lower():
                    logger.warning(f"Geometry package error, trying without geometry: {geometry_error}")
    ...
    def _convert_to_docx(self, input_path: str, output_path: str, input_format: str,
                        options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            elif self.dependencies['python_docx'] and input_format in ['txt', 'html', 'markdown']:
                return self._convert_to_docx_python_docx(input_path, output_path, input_format, options)
            elif self.dependencies['pandoc']:
                return self._convert_to_docx_pandoc(input_path, output_path, input_format, options)
            else:
    ...
    def _convert_to_docx_python_docx(self, input_path: str, output_path: str,
                                    input_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    def _convert_pdf_to_docx(self, input_path: str, output_path: str,
                            options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            success, error = self._extract_text_from_pdf(input_path, temp_txt.name, options)
            if not success:
                os.unlink(temp_txt.name)
            ...
            if self.dependencies['python_docx']:
                try:
                    ...
                    for para in paragraphs:
                        if para.strip():
                            ...
                            if para.isupper() or (len(para) < 100 and para[0].isdigit()):
                    ...
                    if options.get('include_metadata'):
                        ...
                        if options.get('ocr_scan'):
                            doc.add_paragraph('Note: This document was processed with OCR')
                ...
                os.unlink(temp_txt.name)
                return False, "python-docx library not available for PDF to DOCX conversion"
                
        except Exception as e:
            logger.error(f"PDF to DOCX conversion error: {e}")
            return False, f"PDF to DOCX conversion failed: {str(e)}"
    
    def _convert_to_docx_pandoc(self, input_path: str, output_path: str,
                               input_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    def _convert_to_html(self, input_path: str, output_path: str, input_format: str,
                        options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if input_format == 'docx' and self.dependencies['win32com']:
                try:
                    success, error = self._convert_docx_to_html_com(input_path, output_path, options)
                    if success:
                        return True, ""
                    else:
            ...
            if self.dependencies['pandoc']:
                return self._convert_to_html_pandoc_enhanced(input_path, output_path, input_format, options)
            
            # 3. Basic conversion as fallback
            else:
                return self._convert_to_html_basic_enhanced(input_path, output_path, input_format, options)
                
        except Exception as e:
            return False, f"HTML conversion error: {str(e)}"
    
    def _convert_to_html_pandoc_enhanced(self, input_path: str, output_path: str,
                                        input_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if input_format == 'doc':
                ...
                temp_docx = tempfile.NamedTemporaryFile(mode='w+b', suffix='.docx', delete=False)
                temp_docx.close()
                success, error = self._convert_doc_to_docx_com(input_path, temp_docx.name, options)
                if success:
    ...
    def _convert_to_html_basic_enhanced(self, input_path: str, output_path: str,
                                       input_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
        """Enhanced basic HTML conversion without Pandoc"""
        try:
            if input_format == 'txt':
                return self._convert_txt_to_html_enhanced(input_path, output_path, options)
            elif input_format == 'markdown':
                return self._convert_markdown_to_html_enhanced(input_path, output_path, options)
            elif input_format == 'docx':
                return self._convert_docx_to_html_basic(input_path, output_path, options)
            elif input_format == 'doc':
                # Extract text from DOC and convert to HTML
                temp_txt = tempfile.NamedTemporaryFile(mode='w+', suffix='.txt', delete=False)
                temp_txt.close()
                success, error = self._extract_text_from_doc(input_path, temp_txt.name, options)
                if success:
                    result = self._convert_txt_to_html_enhanced(temp_txt.name, output_path, options)
                    os.unlink(temp_txt.name)
                    return result
                else:
                    os.unlink(temp_txt.name)
                ...
                return False, f"No enhanced basic HTML converter available for {input_format}"
                
        except Exception as e:
            logger.error(f"Enhanced basic HTML conversion error: {e}")
            return False, str(e)
    
    def _convert_txt_to_html_enhanced(self, input_path: str, output_path: str, options: Dict[str, Any]) -> Tuple[bool, str]:
            ...
            if para:
                ...
                if len(lines) == 1 and len(para) < 100:
                    if para.isupper() or para.endswith(':'):
                        html_body += f'<h2 class="enhanced-heading level-2">{para}</h2>\n'
                        continue
...
<body>
    ...
    def _convert_markdown_to_html_enhanced(self, input_path: str, output_path: str, options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        toc_section = f'<div class="table-of-contents"><h2>Table of Contents</h2>{toc}</div>' if toc else ''
...
<body>
    ...
    def _basic_markdown_to_html(self, md_content: str) -> str:
        ...
        # Headers
        html = re.sub(r'^# (.*)', r'<h1 class="enhanced-heading level-1">\1</h1>', html, flags=re.MULTILINE)
        html = re.sub(r'^## (.*)', r'<h2 class="enhanced-heading level-2">\1</h2>', html, flags=re.MULTILINE)
        html = re.sub(r'^### (.*)', r'<h3 class="enhanced-heading level-3">\1</h3>', html, flags=re.MULTILINE)
    ...
    def _convert_docx_to_html_basic(self, input_path: str, output_path: str, options: Dict[str, Any]) -> Tuple[bool, str]:
                ...
                if text:
                    ...
                    if paragraph.style.name.startswith('Heading'):
                        ...
                        try:
                            ...
                            html_body += f'<h2 class="enhanced-heading level-2">{text}</h2>\n'
                    else:
...
<body>
    ...
    def _enhance_existing_html(self, input_path: str, output_path: str, options: Dict[str, Any]) -> Tuple[bool, str]:
            ...
            if self.dependencies['bs4']:
                ...
                for i in range(1, 7):
                    for heading in soup.find_all(f'h{i}'):
                        heading['class'] = heading.get('class', []) + [f'enhanced-heading', f'level-{i}']
    ...
    def _convert_to_html_basic(self, input_path: str, output_path: str,
                              input_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
...
<body>
    ...
    def _convert_to_markdown(self, input_path: str, output_path: str, input_format: str,
                           options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if self.dependencies['pandoc']:
                return self._convert_to_markdown_pandoc(input_path, output_path, input_format, options)
            else:
                return self._convert_to_markdown_basic(input_path, output_path, input_format, options)
                
        except Exception as e:
            return False, f"Markdown conversion error: {str(e)}"
    
    def _convert_to_markdown_pandoc(self, input_path: str, output_path: str,
                                   input_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if input_format == 'doc':
                ...
                temp_txt = tempfile.NamedTemporaryFile(mode='w+', suffix='.txt', delete=False)
                temp_txt.close()
                success, error = self._extract_text_from_doc(input_path, temp_txt.name, {})
                if success:
                    try:
                        pypandoc.convert_file(
                            temp_txt.name, 
    ...
    def _convert_to_markdown_basic(self, input_path: str, output_path: str,
                                  input_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            if input_format == 'txt':
                ...
                for i in range(1, 7):
                    for heading in soup.find_all(f'h{i}'):
                        md_lines.append('#' * i + ' ' + heading.get_text().strip() + '\n')
                ...
                return False, f"No basic Markdown converter available for {input_format}"
                
        except Exception as e:
            logger.error(f"Basic Markdown conversion error: {e}")
            return False, str(e)
    
    def _convert_to_rtf(self, input_path: str, output_path: str, input_format: str,
                       options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if self.dependencies['pandoc']:
                return self._convert_to_rtf_pandoc(input_path, output_path, input_format, options)
            else:
                return self._convert_to_rtf_basic(input_path, output_path, input_format, options)
                
        except Exception as e:
            return False, f"RTF conversion error: {str(e)}"
    
    def _convert_to_rtf_pandoc(self, input_path: str, output_path: str,
                              input_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    def _convert_to_rtf_basic(self, input_path: str, output_path: str,
                             input_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
...
\f0\fs24 """ + content.replace('\n', r'\par ') + r"}"
    ...
    def _convert_to_odt(self, input_path: str, output_path: str, input_format: str,
                       options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if self.dependencies['pandoc']:
                return self._convert_to_odt_pandoc(input_path, output_path, input_format, options)
            else:
    ...
    def _convert_to_odt_pandoc(self, input_path: str, output_path: str,
                              input_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    def _convert_to_txt(self, input_path: str, output_path: str, input_format: str,
                       options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if input_format == 'pdf':
                ...
                return self._extract_text_from_markdown(input_path, output_path, options)
            elif self.dependencies['pandoc']:
                return self._convert_to_txt_pandoc(input_path, output_path, input_format, options)
            else:
    ...
    def _extract_text_from_pdf(self, input_path: str, output_path: str,
                              options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if not extracted_text.strip() and options.get('ocr_scan') and self.dependencies['ocr']:
                try:
                    import fitz  # PyMuPDF
    ...
    def _extract_text_from_docx(self, input_path: str, output_path: str,
                               options: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    def _extract_text_from_doc(self, input_path: str, output_path: str,
                              options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            return False, ("DOC format (Word 97-2003) conversion failed. "
    ...
    def _convert_doc_via_docx(self, input_path: str, output_path: str, output_format: str,
                             options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if not success:
                ...
                return self._convert_doc_fallback(input_path, output_path, output_format, options)
            
            # Step 2: Convert DOCX to target format
            try:
                if output_format == 'pdf':
                    result = self._convert_to_pdf(temp_docx.name, output_path, 'docx', options)
                elif output_format == 'docx':
                    # Already DOCX, just copy
                    shutil.copy2(temp_docx.name, output_path)
                    result = (True, "")
                elif output_format == 'html':
                    result = self._convert_to_html(temp_docx.name, output_path, 'docx', options)
                elif output_format == 'markdown':
                    result = self._convert_to_markdown(temp_docx.name, output_path, 'docx', options)
                elif output_format == 'rtf':
                    result = self._convert_to_rtf(temp_docx.name, output_path, 'docx', options)
                elif output_format == 'odt':
                    result = self._convert_to_odt(temp_docx.name, output_path, 'docx', options)
                else:
                ...
                os.unlink(temp_docx.name)
                logger.error(f"DOCX to {output_format} conversion failed: {conversion_error}")
                return False, f"Conversion from DOCX to {output_format} failed: {str(conversion_error)}"
            
        except Exception as e:
            logger.error(f"DOC via DOCX conversion error: {e}")
            return False, f"DOC conversion error: {str(e)}"
    
    def _convert_doc_to_docx_com(self, input_path: str, output_path: str, options: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    def _convert_doc_fallback(self, input_path: str, output_path: str, output_format: str,
                             options: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            try:
                if output_format == 'pdf':
                    result = self._convert_to_pdf(temp_txt.name, output_path, 'txt', options)
                elif output_format == 'html':
                    result = self._convert_to_html(temp_txt.name, output_path, 'txt', options)
                elif output_format == 'markdown':
                    result = self._convert_to_markdown(temp_txt.name, output_path, 'txt', options)
                elif output_format == 'rtf':
                    result = self._convert_to_rtf(temp_txt.name, output_path, 'txt', options)
                elif output_format == 'docx':
                    result = self._convert_to_docx(temp_txt.name, output_path, 'txt', options)
                elif output_format == 'odt':
                    result = self._convert_to_odt(temp_txt.name, output_path, 'txt', options)
                else:
    ...
    def _convert_docx_to_pdf_com(self, input_path: str, output_path: str, options: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    def _convert_docx_to_html_com(self, input_path: str, output_path: str, options: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    def _enhance_com_generated_html(self, html_path: str, options: Dict[str, Any]):
        ...
        try:
            ...
            if self.dependencies['bs4']:
                ...
                for element in soup.find_all(attrs={'class': lambda x: x and ('Mso' in str(x) or 'mso' in str(x))}):
                    element.extract()
...
"""
    ...
    def _extract_text_from_html(self, input_path: str, output_path: str,
                               options: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    def _extract_text_from_markdown(self, input_path: str, output_path: str,
                                   options: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    def _convert_to_txt_pandoc(self, input_path: str, output_path: str,
                              input_format: str, options: Dict[str, Any]) -> Tuple[bool, str]:
